...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        self.fpn_params["lateral_p3"] = nn.Conv2d(
        ...
        self.fpn_params["lateral_p4"] = nn.Conv2d(
        ...
        self.fpn_params["lateral_p5"] = nn.Conv2d(
        ...
        self.fpn_params["p3+p4+p5"] = nn.Sequential(
          nn.Conv2d(
        ...
        self.fpn_params["p4+p5"] = nn.Conv2d(
          in_channels=self.out_channels,
          out_channels=self.out_channels,
          kernel_size=3,
          stride=1,
    ...
    def forward(self, images: torch.Tensor):
        ...
        # Replace "PASS" statement with your code
        p5 = self.fpn_params["p5"](self.fpn_params["lateral_p5"](backbone_feats['c5']));
        p4 = self.fpn_params["p4+p5"](self.fpn_params["lateral_p4"](backbone_feats['c4']))+F.interpolate(p5,scale_factor=2);
        p3 = self.fpn_params["p3+p4+p5"](self.fpn_params["lateral_p3"](backbone_feats['c3']));
        p3 = self.fpn_params["p3+p4+p5"](self.fpn_params["lateral_p3"](backbone_feats['c3']))+F.interpolate(p4,scale_factor=2);
        fpn_feats["p3"] = p3;
        fpn_feats["p4"] = p4;
        fpn_feats["p5"] = p5;
        ######################################################################
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        # Replace "PASS" statement with your code
        H,W = feat_shape[-2:]
        x = (torch.arange(0, W, dtype=dtype, device=device)+0.5)*level_stride;
        y = (torch.arange(0, H, dtype=dtype, device=device)+0.5)*level_stride;
        x,y = torch.meshgrid(x,y)
        x_flatten = x.flatten()
        y_flatten = y.flatten()
        locations = torch.stack([x_flatten, y_flatten], dim=1)
...
class FCOSPredictionNetwork(nn.Module):
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level in feats_per_fpn_level:
          feat_map = feats_per_fpn_level[level]
          cls_pred = self.pred_cls(self.stem_cls(feat_map))
          box_pred = self.pred_box(self.stem_box(feat_map))
          ctr_pred = self.pred_ctr(self.stem_box(feat_map))
          class_logits[level] = cls_pred.permute(0, 2, 3, 1).reshape(feat_map.shape[0], -1,cls_pred.shape[1])
          boxreg_deltas[level] = box_pred.permute(0, 2, 3, 1).reshape(feat_map.shape[0], -1, 4)
          centerness_logits[level] = ctr_pred.permute(0, 2, 3, 1).reshape(feat_map.shape[0], -1, 1)
...
def fcos_get_deltas_from_locations(
    ...
    # Replace "PASS" statement with your code
    N = gt_boxes.size(0)
    deltas = torch.zeros((N,4),dtype=torch.float32, device=gt_boxes.device)
    bgmask = (gt_boxes[:, 0] == -1)
    deltas[bgmask, :] = -1
    fgmask =(gt_boxes[:, 0] != -1)
    locations = torch.cat((locations,locations),1)
    gt_no_lable = torch.narrow(gt_boxes,1,0,4)
    deltas[fgmask,0:2] = (locations[fgmask,0:2]-gt_no_lable[fgmask,0:2])/stride
    deltas[fgmask,2:4] = (gt_no_lable[fgmask,2:4]-locations[fgmask,2:4])/stride
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_apply_deltas_to_locations(
    ...
    N = deltas.size(0)
    output_boxes = torch.zeros((N, 4), dtype=torch.float32, device=deltas.device)
    x = locations[:, 0]
    y = locations[:, 1]
    l = deltas[:, 0].clamp(min=0) * stride
    t = deltas[:, 1].clamp(min=0) * stride
    r = deltas[:, 2].clamp(min=0) * stride
    b = deltas[:, 3].clamp(min=0) * stride
    x1 = x - l
    y1 = y - t
    x2 = x + r
    y2 = y + b
    output_boxes[:, 0] = x1.clamp(min=0)
    output_boxes[:, 1] = y1.clamp(min=0)
    output_boxes[:, 2] = x2
    output_boxes[:, 3] = y2
    ##########################################################################
    #                             END OF YOUR CODE                           #
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    # Replace "PASS" statement with your code
    N = deltas.shape[0]
    centerness_targets = torch.zeros((N,), dtype=deltas.dtype, device=deltas.device)

    for i in range(N):
        l, t, r, b = deltas[i]
        if l == -1:
            centerness_targets[i] = -1
        else:
            centerness_targets[i] = torch.sqrt((torch.min(l, r) / torch.max(l, r)) * (torch.min(t, b) / torch.max(t, b)))
